...
/* Récupération d'une connexion depuis la Factory */
connexion = daoFactory.getConnection();
preparedStatement = initialisationRequetePreparee( connexion,SQL_SELECT_PAR_NUM_SECURITE, false, numero_securite );
resultSet = preparedStatement.executeQuery();
/* Parcours de la ligne de données de l'éventuel ResulSet
retourné */
if ( resultSet.next() ) {
...
/* Récupération d'une connexion depuis la Factory */
connexion = daoFactory.getConnection();
preparedStatement = initialisationRequetePreparee( connexion,SQL_INSERT, true,patient.getNom(),patient.getPrenom(), patient.getAdresse(),patient.getTelephone(),patient.getEmail(),patient.getGroupeSanguin(), patient.getSexe(),patient.getNum_assurance(),patient.getMotDePasse());
