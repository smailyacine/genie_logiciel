...
l'interface UtilisateurDao */
	private static final String SQL_INSERT_DOCTEUR = "insert into p_cliniuqe.rdv (id_docteur,rdvTime,idslot) VALUES(?,?,?)";
	...
	private static final String SQL_SELECT_RDV_PAR_PATIENT = "SELECT * FROM p_cliniuqe.rdv as R inner join p_cliniuqe.docteur as D "
			+ "on R.id_docteur = D.identifiant  inner join  p_cliniuqe.patient as P on R.id_patient = P.num_assurance "
			+ "inner join p_cliniuqe.slot as S on R.idslot = S.idslot "
			+ "where R.id_patient = ? and isreserved = 1;";
	private static final String SQL_ANNULER_PATIENT = "update p_cliniuqe.rdv SET id_patient = null , isreserved = 0 where id = ?;";
	private static final String SQL_SUPPRIMER_CRENEAU = "delete from p_cliniuqe.rdv where id_docteur = ? and date(rdvTime) =? and idslot= ? ";
...
public ArrayList<Rdv> trouverRdv( String patientidentifian) throws DAOException {
Connection connexion = null;
PreparedStatement preparedStatement = null;
ResultSet resultSet = null;
...
try{
connexion = daoFactory.getConnection();
preparedStatement = initialisationRequetePreparee( connexion,SQL_SELECT_RDV_PAR_PATIENT, false, patientidentifian);
resultSet = preparedStatement.executeQuery();
/* Parcours de la ligne de données de l'éventuel ResulSet
retourné */
while ( resultSet.next() ) {
rdv.add(map_rdv( resultSet ));
}
} catch ( SQLException e ) {
throw new DAOException( e );
} finally {
...
UtilisateurDao */
@Override
public void creerRdv_docteur(String identifiant_docteur,String date, Long id_creneau) throws DAOException {
Connection connexion = null;
PreparedStatement preparedStatement = null;
...
/* Récupération d'une connexion depuis la Factory */
connexion = daoFactory.getConnection();
preparedStatement = initialisationRequetePreparee( connexion,SQL_INSERT_DOCTEUR, false ,identifiant_docteur,date,id_creneau);
...
public void annulerRdv(Long id) {
	...
	try {
		/* Récupération d'une connexion depuis la Factory */
		connexion = daoFactory.getConnection();
		preparedStatement = initialisationRequetePreparee( connexion,SQL_ANNULER_PATIENT, false, id);
		int statut = preparedStatement.executeUpdate();
		if ( statut != 1 ) {
...
@Override
public void supprimerCreneau(String identifiant, Long id, String date) {
	Connection connexion = null;
	PreparedStatement preparedStatement = null;
	try {
		/* Récupération d'une connexion depuis la Factory */
		connexion = daoFactory.getConnection();
		preparedStatement = initialisationRequetePreparee( connexion,SQL_SUPPRIMER_CRENEAU, false, identifiant,date,id );
		int statut = preparedStatement.executeUpdate();
		if ( statut != 1 ) {
			throw new DAOException( "Échec de la suppression de patient, aucune ligne supprimer dans la table." );
			}
